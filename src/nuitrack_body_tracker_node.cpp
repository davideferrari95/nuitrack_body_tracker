...
namespace nuitrack_body_tracker {
  ...
  public:
    ...
    nuitrack_body_tracker_node(std::string name) : _name(name) {
      ...
      body_tracking_array_pub_ = nh_.advertise<body_tracker_msgs::BodyTrackerArray>("body_tracker_array/position", 1);
    ...
    ~nuitrack_body_tracker_node() {ROS_INFO("nuitrack_body_tracker_node shutting down");}
          ...
          for (boost::property_tree::ptree::value_type &instance : root.get_child("Instances"))
          {
            ...
            for (boost::property_tree::ptree::value_type &found_object : instance.second)
            {
              ...
              else if ("face" == found_object.first)
              {
                ...
                {
                  std::cout << "FACE ID (" << json_id << ") DOES NOT MATCH SKELETON (" << skeleton.id << ")... SKIPPING  (or object != Human?)" << std::endl;
                }
                else
                {
